...
parser.add_argument("--maxepochs", type=int, default=100, help='max number of epochs')
...
class Thoegaze(nn.Module):
    ...
    def forward(self, input ,decoder_input=None, state=None):

        x,c=input


        # if decoder_inputs:
        decoder_input=self._shift_right(decoder_input)
        ...
        _t = self.t_encoder(inputs_embeds=c,return_dict=True).last_hidden_state
        if self.pooling_type=='max':
            _t=self.max_pool(_t)
        elif self.pooling_type=='gru':
            _,ht=self.gru(_t)
        ...
        if self.use_transcription_loss:
            ...
            transcription_out=self.linear(self.s_decoder(input_ids=decoder_input,encoder_hidden_states=_s,encoder_attention_mask=mem_mask,use_cache=True,return_dict=False
            )[0])

        if args.auto_regrssion_decoder:
            x=nn.functional.pad(x[:,:-1,:],(0,0,1,0,0,0),'constant',value=0)
            
            p= self.decoder(inputs_embeds=x,encoder_hidden_states=_s+_t,return_dict=True).last_hidden_state
...
def criterion(outputs,y,t=None,alpha=0.5,beta=1,gamma=1,step=None):
    loss={}
    para={}
    ...
    if args.vq:
        ...
        para['commitment'] = 0.5
        para['codebook'] = 1
        loss['commitment']=ls['commitment'] 
        loss['codebook']=ls['codebook']
    ...
    if t:
        ...
        para['trans']=gamma
        loss['trans'] = sfn(torch.transpose(_t, -2, -1),t) 
...
def train(epoch):
    ...
    for batch in train_dataloader:
        ...
        if args.usetrans:
            ...
            outputs = model([x,c],[t])
            loss= criterion(outputs, y,t,alpha=args.alpha,beta=args.beta,gamma=args.gamma)
        else:

            outputs = model([x,c])
            loss= criterion(outputs, y,None,alpha=args.alpha,beta=args.beta,gamma=args.gamma)
        optimizer.zero_grad()
...
def parse_fn(features):
    ...
    features['x'] =np.frombuffer(
        features['x'], dtype=np.float32).reshape(args.segwidth, -1)
    features['c'] =np.frombuffer(
        features['c'], dtype=np.float32).reshape(args.segwidth, -1)
    features['y'] =np.frombuffer(
        features['y'], dtype=np.float32).reshape(args.segwidth, -1)

    # if features['inputs_embeds'].shape[-1] not in [512, nbins]:
    #     print(features['inputs_embeds'].shape, features['labels'])
    #     assert 1==2
    if args.usetrans:
        _features=eval(features['t'])

        features['t']=[1]
        for i,x in enumerate(_features):
            if not x==[]:
                features['t'].append(i+3)
                for y in x:
                    features['t'].append(y+3+args.segwidth)
        del _features
    
        if len(features['t'])>int(1.5*args.segwidth-1):
...
if __name__=="__main__":
    ...
    description = {"x": "byte", "c":"byte","t": "byte", "t":"byte"}
    ...
    print("training %s for context thoegazer..." % 'transformer ')
    since = time.time()
    es=0
    best_loss=1e100
