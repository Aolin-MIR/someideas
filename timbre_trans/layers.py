...
def get_attn_pad_mask(seq_q, seq_k, pad_index):
    ...
    return torch.ones(batch_size,len_q,len_k).to(device)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, d_ff, d_k, d_v, n_heads,dropout=0):
...
class Encoder(nn.Module):
    ...
    def __init__(self, d_model, d_ff, d_k, d_v, n_heads, n_layers, pad_index,dropout=0):
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            x, enc_self_attn = layer(x, enc_self_attn_mask)
