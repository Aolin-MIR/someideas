...
class ClassifierGenerator(Sequence):
    def __init__(self, path_pos, path_label, B, T=40, min_count=1, shuffle=True):
        self.PAD = 0
        self.BOS = 1
        self.EOS = 2
        self.UNK = 3
    ...
    def __getitem__(self, idx):
        ...
        for i in range(start, end):
            ...
            label = linecache.getline(self.path_label,idx)
    ...
    def reset(self):
        ...
        self.indicies =pos_indices# np.concatenate([pos_indices, neg_indices])
        if self.shuffle:
...
class DiscriminatorGenerator(Sequence):
    ...
    def reset(self):
        ...
        pos_indices = np.arange(start=1, stop=self.n_data_pos+1)
        # if self.shuffle:
        #     random.shuffle(self.pos_indicies)
        # self.pos_indicies=self.pos_indicies[:self.n_data_neg]
        neg_indices = -1 * np.arange(start=1, stop=self.n_data_neg+1)
